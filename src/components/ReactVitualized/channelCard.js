...
const Img = ({ className, alt, src, onError }) => {
    ...
    return (
        <img src={res.src} className={className} alt={alt} onError={onError} />
    );
};
...
const ChannelCard = ({ channel }) => {
    ...
    const imgErrorHandler = (e) => {
        if (
            e.target.src !==
            "https://picsum.photos/600/400.jpg?blur=6&grayscale"
        ) {
            // this prevents infinite loop error in cases where the backup image itself is down
    ...
    return (
        <div className="group cursor-pointer rounded shadow-md hover:shadow-2xl w-full h-full bg-gray-100">
            ...
            <h5 className="group-hover:text-gray-900 text-gray-700 font-medium p-3">
                {channel.name}
            </h5>
        </div>
    );
